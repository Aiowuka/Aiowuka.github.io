...
export default async function Home() {
  ...
  return <div className="page">
    <header className="site-header"><Link className="name" href="/">AIowuka</Link><nav className="nav"><a href="#about">关于 / About</a><a href="#now">近况 / Now</a><Link href="/private">私人区域 / Private</Link>{profile ? <Link href={profile.role==='owner'?'/admin':'/private'}>{profile.display_name || 'Account'}</Link> : <Link href="/login">登录 / Sign in</Link>}</nav></header>
    <main>
